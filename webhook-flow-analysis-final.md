...
6. 注意 `X-Plane-Delivery` 在每次重试时保持不变（同一事件的多次重试使用相同 ID）

### 📌 运维安全建议

1. 生产环境必须配置 `WEBHOOK_DISALLOWED_DOMAINS`，将 Plane 实例域名加入黑名单，缓解 PATCH 回环防护失效问题
2. 监控 `webhook_task.py:326` 和 `:382` 的静默失败日志
3. 重要业务建议在接收方实现 ACK 确认机制
4. 密钥轮换前建议先暂停相关 Webhook，等待在途任务完成
