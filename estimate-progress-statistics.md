...
前端枚举定义在 [estimates.ts](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/packages/constants/src/estimates.ts#L12-L16)：
...
export enum EEstimateSystem {
  ...
  TIME = "time",            // 时间/小时（EE 功能）
}
```

后端模型定义在 [estimate.py](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/api/plane/db/models/estimate.py#L13-L16)：
...
> **注意**：后端 `EstimateType` 只声明了 `CATEGORIES` 和 `POINTS` 两种，**没有 `TIME`**。`TIME` 是前端 EE（企业版）扩展的类型，后端创建时若传入 `type="time"` 可存入 CharField，但后端统计逻辑 **不识别 time 类型**。

### 1.2 项目绑定 Estimate

Project 模型通过 ForeignKey 关联 Estimate（[project.py](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/api/plane/db/models/project.py#L109)）：
...
前端判断项目是否启用了 Estimate（[project-estimate.store.ts](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/web/core/store/estimates/project-estimate.store.ts#L135-L140)）：
...
每个 Estimate 下挂多个 EstimatePoint（[estimate.py](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/api/plane/db/models/estimate.py#L43-L47)）：

| 字段      | 类型          | 说明                                                         |
| --------- | ------------- | ------------------------------------------------------------ |
| `key`     | IntegerField  | 序号（0 起步，MinValueValidator(0)），用于排序              |
| `value`   | CharField(255) | 显示值：points 型存 "1"/"2"/"5"/"8"；categories 型存 "XS"/"S"/"M"；time 型存分钟数 "60"/"120" |
| `estimate`| ForeignKey    | 所属 Estimate                                                |

前端预设模板（[estimates.ts](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/packages/constants/src/estimates.ts#L29-L141)）：
...
Issue 有两个相关字段（[issue.py](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/api/plane/db/models/issue.py#L128-L134)）：
...
- `point`：**旧版字段**，整数 0-12，已废弃
- `estimate_point`：**当前使用字段**，FK 指向 EstimatePoint，`SET_NULL` 表示删除 point 时置空

### 2.2 前端录入组件

Issue 的 estimate 通过 [estimate.tsx](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/web/core/components/dropdowns/estimate.tsx#L97-L130) 下拉选择：
...
前端只读展示（[readonly/estimate.tsx](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/web/core/components/readonly/estimate.tsx#L38-L41)）：
...
3. **TIME** 型：`value` 存分钟数字符串（"60", "120"），前端展示时 `convertMinutesToHoursMinutesString` 将分钟→"2h 0m"

### 3.2 TIME 类型的 value 存的是分钟

`convertMinutesToHoursMinutesString`（[datetime.ts](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/packages/utils/src/datetime.ts#L367-L371)）：
...
这是 **展示层** 的格式化，不是分制间的换算。TIME 类型在数据库中统一存分钟数。

### 3.3 后端统计只认 `type="points"`

这是 **最关键的发现**。后端在所有 estimate 汇总查询中，都硬编码了 `estimate_point__estimate__type="points"` 过滤条件：

- **CycleProgressEndpoint**（[cycle/base.py](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/api/plane/app/views/cycle/base.py#L665-L666)）：
  ```python
...
- **ModuleViewSet.get_queryset**（[module/base.py](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/api/plane/app/views/module/base.py#L147-L148)）：
  ```python
  Issue.issue_objects.filter(
      estimate_point__estimate__type="points",
      ...
...
- **CycleAnalyticsEndpoint**（[cycle/base.py](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/api/plane/app/views/cycle/base.py#L832-L837)）：
  ```python
...
- **burndown_plot**（[analytics_plot.py](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/api/plane/utils/analytics_plot.py#L127-L132)）：
  ```python
  estimate_type = Project.objects.filter(
      workspace__slug=slug,
      pk=project_id,
...
[EstimateTypeDropdown](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/web/core/components/cycles/dropdowns/estimate-type-dropdown.tsx#L30-L31)：
...
[CycleSidebarDetails](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/web/core/components/cycles/analytics-sidebar/sidebar-details.tsx#L52-L58) 也只对 POINTS 类型显示 estimate 点数：
...
[getDistributionDataOfIssue](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/packages/utils/src/distribution-update.ts#L104-L109)：
...
[CycleProgressEndpoint](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/api/plane/app/views/cycle/base.py#L658-L783) 每次请求时从数据库实时聚合：
...
- `Issue.issue_objects` → 自动排除 triage 状态、已归档、草稿 issue（[IssueManager](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/api/plane/db/models/issue.py#L92-L101)）
...
前端在展示时优先使用快照数据（[issue-progress.tsx](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/web/core/components/cycles/analytics-sidebar/issue-progress.tsx#L45-L58)）：
...
[calculateCycleProgress](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/packages/utils/src/cycle.ts#L202-L245)：
...
[formatV2Data](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/packages/utils/src/cycle.ts#L139-L178) 根据 `estimateType` 切换数据源：
...
后端 burndown_plot（[analytics_plot.py](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/api/plane/utils/analytics_plot.py#L123-L264)）在 `plot_type="points"` 时：
- 查询所有 `estimate_point__isnull=False` 的 issue（注意：**没有** `estimate__type="points"` 过滤！）
- 将 `estimate_point__value` 转为 float 并按日累加
- 累计待完成 = 总 estimate_points - 截至当日的已完成 estimate_points

> **差异**：`burndown_plot` 中的 `estimate_point__isnull=False` 过滤条件不同于 `CycleProgressEndpoint` 的 `estimate_point__estimate__type="points"`。这意味着 burndown 图可能包含 TIME/CATEGORIES 类型的 estimate_point，但 CycleProgressEndpoint 不会。这可能导致 **燃尽图与进度条数据不一致**。
...
[ModuleViewSet.get_queryset](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/api/plane/app/views/module/base.py#L145-L277) 对每个状态组分别做子查询聚合：
...
[ModuleViewSet.retrieve](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/api/plane/app/views/module/base.py#L417-L428) 先判断项目是否有 points 型 estimate：
...
只有 `estimate_type=True` 时才查询 `estimate_distribution`（按 assignee/label 分组的 estimate 聚合）和 burndown 图。

### 6.3 前端 Module 进度

[ModuleAnalyticsProgress](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/web/core/components/modules/analytics-sidebar/issue-progress.tsx#L70-L78)：
...
Module 的 "points" 切换下拉只在 `isCurrentEstimateTypeIsPoints`（estimate type 为 POINTS）时显示（[issue-progress.tsx](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/web/core/components/modules/analytics-sidebar/issue-progress.tsx#L135)）。
...
[analytics_plot.py](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/api/plane/utils/analytics_plot.py#L110-L111) 的 `build_graph_plot`：
...
这里 **没有** `estimate_point__estimate__type="points"` 过滤。所以 Analytics 图表会包含所有类型的 estimate_point 值（包括 CATEGORIES 和 TIME），但对 CATEGORIES 型 `parseFloat("XS")` 结果是 NaN，Sum 后可能为 null。

### 7.2 前端 Analytics 选择器

[select-y-axis.tsx](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/web/core/components/analytics/select/select-y-axis.tsx) 提供 "issue_count" 和 "estimate" 两个 Y 轴选项。
...
Cycle 的 [calculateCycleProgress](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/packages/utils/src/cycle.ts#L234-L235)：
```ts
...
**Cycle**：`total_issues` 的 Count 过滤条件（[cycle/base.py](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/api/plane/app/views/cycle/base.py#L115-L125)）中 **没有** `parent__isnull=True` 过滤。所以 **子任务计入** `total_issues` 和 `total_estimate_points`。

**Module**：`total_issues` 的子查询（[module/base.py](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/api/plane/app/views/module/base.py#L136-L144)）也 **没有** `parent__isnull=True` 过滤。子任务同样计入。

**CycleProgressEndpoint** 的 estimate 聚合（[cycle/base.py](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/api/plane/app/views/cycle/base.py#L664-L711)）也 **没有** 排除子任务。
...
[IssueManager](file:///d:/fz/0508-3/solo-dogfeeding/code/196-plane/apps/api/plane/db/models/issue.py#L92-L101)：
...
方式2: CycleAnalyticsEndpoint.get?type=points
  → 返回 assignee/label 维度的 estimate 分布 + burndown completion_chart
...
### 10.2 burndown_plot 与 CycleProgressEndpoint 过滤条件不一致

- `CycleProgressEndpoint`: `estimate_point__estimate__type="points"` + `estimate_point__isnull` 无过滤
- `burndown_plot`: `estimate_point__isnull=False`，**没有** `estimate__type="points"` 过滤

如果项目同时存在 points 和 time 型 estimate_point，burndown 图会包含 time 类型的值，但进度条不会。
